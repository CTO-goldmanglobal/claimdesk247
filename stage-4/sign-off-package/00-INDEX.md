...
| **2d-VIC** | **Property Damage — VIC** | `stage-3/app/data/rule-tree.vic.pd.v1.json` | IX-18..21 | **[STAGE-1 UNSIGNED]** Counsel memo §2.2 = VERIFY. Un-signed 2026-07-14 (review P1-6); G-PROD-LOCK escalates every VIC PD intake as `unsigned-scenario`. Stage-2 re-sign pending: VIC licensing verify + citation audit (P1-3). |
| **2d-QLD** | **Property Damage — QLD** | `stage-3/app/data/rule-tree.qld.pd.v1.json` | IX-23..25 | **[STAGE-1 UNSIGNED]** Counsel memo §2.2 = CONDITIONAL GO ("do not launch commercially until resolved"). Un-signed 2026-07-14; G-PROD-LOCK escalates. Stage-2 re-sign pending: QLD OFT position + citation audit. |
| **2d-WA** | **Property Damage — WA** | `stage-3/app/data/rule-tree.wa.pd.v1.json` | IX-23..25 | **[STAGE-1 UNSIGNED]** VERIFY. Un-signed 2026-07-14; G-PROD-LOCK escalates. Stage-2 re-sign pending: WA licensing verify + citation audit. |
| **2d-SA** | **Property Damage — SA** | `stage-3/app/data/rule-tree.sa.pd.v1.json` | IX-26 | **[STAGE-1 UNSIGNED]** VERIFY. Stage-2 re-sign pending: SA licensing verify + citation audit. |
| **2d-TAS** | **Property Damage — TAS** | `stage-3/app/data/rule-tree.tas.pd.v1.json` | IX-26 | **[STAGE-1 UNSIGNED]** VERIFY. Stage-2 re-sign pending: TAS licensing verify + citation audit. |
| **2d-ACT** | **Property Damage — ACT** | `stage-3/app/data/rule-tree.act.pd.v1.json` | IX-26 | **[STAGE-1 UNSIGNED]** VERIFY. Stage-2 re-sign pending: ACT licensing verify + citation audit. |
| **2d-NT** | **Property Damage — NT** | `stage-3/app/data/rule-tree.nt.pd.v1.json` | IX-26 | **[STAGE-1 UNSIGNED]** VERIFY. 3-year limitation (Limitation Act 1981) flagged in tree + enforced engine-side. Stage-2 re-sign pending: NT licensing verify + citation audit. |
...
>
> **Multi-state national PD (2026-07-13) — SIGNED.** Legal Head go-ahead recorded; all AU PD trees are `live=true` (VIC/QLD/WA/SA/TAS/ACT/NT + NSW). Engine canonicalises `<state>-pd*` scenario ids into shared PD band helpers. Injury firewall still holds in every state. Motor/PL/med-neg remain NSW-only. NSW hashes unchanged. Verify: `python3 stage-4/scripts/sign_rule_trees.py --verify`.
