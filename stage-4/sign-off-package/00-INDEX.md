...
| **2d-VIC** | **Property Damage rule tree — VIC (staging-unsigned, multi-state rollout)** | `stage-3/app/data/rule-tree.vic.pd.v1.json` + `handoff/MULTI-STATE-ROLLOUT-PLAN.md` | `/healthz` rule_tree_versions + IX-18..22 green | **[PENDING — LEGAL]** VIC PD tree cloned from NSW PD with VIC road-rule labels (`Road Safety Road Rules 2017 (Vic)`) and `Limitation of Actions Act 1958 (Vic)` wording. **All 7 scenarios ship UNSIGNED** so the engine escalates every VIC PD intake as `unsigned-scenario` (G-PROD-LOCK) until Legal Head signs. Sign-off prerequisites (per MULTI-STATE-ROLLOUT-PLAN.md §4): (1) **CD-R2 claim-farming memo for VIC** — confirm PD-only Lane 1 is exempt (Victoria has no PD-specific statute; the `Wrongs Act 1958` is personal-injury only); (2) **PD counsel memo for VIC** — agent licensing under Consumer Affairs Victoria, ACL/CHOICE disclosures, fee structure; (3) Legal Head review of scenario content. Sign with `python3 stage-4/scripts/sign_rule_trees.py --tree VIC.property_damage --by "..." --date ...`. |
| **2d-QLD** | **Property Damage rule tree — QLD (staging-unsigned, multi-state rollout)** | `stage-3/app/data/rule-tree.qld.pd.v1.json` + `handoff/MULTI-STATE-ROLLOUT-PLAN.md` | `/healthz` rule_tree_versions + IX-23..25 green | **[PENDING — LEGAL]** QLD PD tree cloned from VIC/NSW PD with QLD road-rule labels (`Transport Operations (Road Use Management—Road Rules) Regulation 2009 (Qld)`) and `Limitation of Actions Act 1974 (Qld)` wording. **All 7 scenarios UNSIGNED**. Prerequisites: (1) **CD-R2 for QLD** — confirm PD-only Lane 1 sits outside *Personal Injuries Proceedings Act 2002* claim-farming/touting; (2) **PD counsel memo for QLD**; (3) Legal Head review. Sign with `python3 stage-4/scripts/sign_rule_trees.py --tree QLD.property_damage --by "..." --date ...`. |
| **2d-WA** | **Property Damage rule tree — WA (staging-unsigned, multi-state rollout)** | `stage-3/app/data/rule-tree.wa.pd.v1.json` + `handoff/MULTI-STATE-ROLLOUT-PLAN.md` | `/healthz` rule_tree_versions + IX-23..25 green | **[PENDING — LEGAL]** WA PD tree cloned from VIC/NSW PD with WA road-rule labels (`Road Traffic Code 2000 (WA)`) and `Limitation Act 2005 (WA)` wording. **All 7 scenarios UNSIGNED**. Prerequisites: (1) **CD-R2 for WA**; (2) **PD counsel memo for WA**; (3) Legal Head review. Sign with `python3 stage-4/scripts/sign_rule_trees.py --tree WA.property_damage --by "..." --date ...`. |
...
>
> **Multi-state rollout (2026-07-13) — see `handoff/MULTI-STATE-ROLLOUT-PLAN.md`.** Engineering scaffold landed for **VIC + QLD + WA** PD (items 2d-VIC / 2d-QLD / 2d-WA). All three ship **unsigned**; G-PROD-LOCK escalates every non-NSW PD intake until Legal Head signs. Intake dropdown accepts NSW/VIC/QLD/WA for motor+PD; unregistered states and `outside_nsw` still escalate at the state-scope guard. NSW sign-offs and hashes are unchanged (IX-21 / IX-25). Personal injury remains escalated to a law-firm partner in every state. Each new state requires its own CD-R2 memo + PD counsel memo + Legal Head sign-off — see MULTI-STATE-ROLLOUT-PLAN.md §4.
