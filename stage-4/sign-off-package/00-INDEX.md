...
>
> **Multi-state rollout (2026-07-13) — see `handoff/MULTI-STATE-ROLLOUT-PLAN.md`.** Engineering scaffold landed for VIC PD as the first proof state (item 2d-VIC). The VIC PD tree ships **unsigned**; G-PROD-LOCK escalates every VIC PD intake until Legal Head signs. NSW sign-offs and hashes are unchanged (per IX-21: motor `3964e668`, PD `1f13febaf6c0`, PL `dc824c3559ab`, med-neg `4e3fe60ac4c2` before and after the scaffold). Personal injury remains escalated to a law-firm partner in every state. Each new state requires its own CD-R2 memo + PD counsel memo + Legal Head sign-off — see the per-state checklist in MULTI-STATE-ROLLOUT-PLAN.md §4.
