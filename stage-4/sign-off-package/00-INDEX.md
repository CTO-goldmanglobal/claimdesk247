...
| **2d-VIC** | **Property Damage — VIC (pre-launch, priority)** | `stage-3/app/data/rule-tree.vic.pd.v1.json` | IX-18..21 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** Road Safety Road Rules 2017 (Vic); Limitation of Actions Act 1958 (Vic) 6 yr. Unsigned → `unsigned-scenario` until signed. Priority sign order #1 after NSW. `sign_rule_trees.py --tree VIC.property_damage`. |
| **2d-QLD** | **Property Damage — QLD (pre-launch, priority)** | `stage-3/app/data/rule-tree.qld.pd.v1.json` | IX-23..25 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** TORUM Road Rules Reg 2009 (Qld); Limitation of Actions Act 1974 (Qld) 6 yr. CD-R2 must address PIP Act 2002. Priority #2. `sign_rule_trees.py --tree QLD.property_damage`. |
| **2d-WA** | **Property Damage — WA (pre-launch, priority)** | `stage-3/app/data/rule-tree.wa.pd.v1.json` | IX-23..25 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** Road Traffic Code 2000 (WA); Limitation Act 2005 (WA) 6 yr. Priority #3. `sign_rule_trees.py --tree WA.property_damage`. |
| **2d-SA** | **Property Damage — SA (pre-launch)** | `stage-3/app/data/rule-tree.sa.pd.v1.json` | IX-26 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** ARR as applied in SA; Limitation of Actions Act 1936 (SA) 6 yr. `sign_rule_trees.py --tree SA.property_damage`. |
| **2d-TAS** | **Property Damage — TAS (pre-launch)** | `stage-3/app/data/rule-tree.tas.pd.v1.json` | IX-26 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** Road Rules 2019 (Tas); Limitation Act 1974 (Tas) 6 yr. `sign_rule_trees.py --tree TAS.property_damage`. |
| **2d-ACT** | **Property Damage — ACT (pre-launch)** | `stage-3/app/data/rule-tree.act.pd.v1.json` | IX-26 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** Road Transport (Road Rules) Regulation 2017 (ACT); Limitation Act 1985 (ACT) 6 yr. `sign_rule_trees.py --tree ACT.property_damage`. |
| **2d-NT** | **Property Damage — NT (pre-launch)** | `stage-3/app/data/rule-tree.nt.pd.v1.json` | IX-26 | **[ENGINEERING COMPLETE — AWAITING LEGAL HEAD PRE-LAUNCH SIGN-OFF]** ARR as applied in NT; **Limitation Act 1981 (NT) — 3 years** (shorter — flagged in tree). `sign_rule_trees.py --tree NT.property_damage`. |
...
>
> **Multi-state national PD (2026-07-13) — see `handoff/MULTI-STATE-ROLLOUT-PLAN.md`.** Engineering is **complete** for PD in every Australian jurisdiction (NSW live + VIC/QLD/WA priority + SA/TAS/ACT/NT). This is **not** an internal-review stub: intake accepts all AU states; each non-NSW PD tree is content-complete and hash-isolated. Trees remain **unsigned** by design — G-PROD-LOCK holds bands until **Legal Head approves before launch** (sign order: VIC → QLD → WA → SA → TAS → ACT → NT). Per-state CD-R2 + PD counsel memo still required. Motor/PL/med-neg stay NSW-only. Personal injury always escalates to a law-firm partner. NSW hashes unchanged (IX-21 / IX-25 / IX-26).
